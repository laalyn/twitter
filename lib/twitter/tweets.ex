defmodule Twitter.Tweets do
  ...
  def stream_tweets!() do
    try do
      ...
      case err do
        %RuntimeError{message: "db is full"} ->
          raise err
        %RuntimeError{message: "reset stream"} ->
          ANSI.format([:black_background, ANSI.format([:yellow, "[tweets] reset the stream"])])
          |> IO.puts
      ...
      try do
        ...
        {{last_insert_cycles} = insert_timer, {tweets, tweet_deletes, {users_w_both, users_wo_followers, users_wo_following, users_wo_both} = users, places} = inserts, {user_id_lookup, place_id_lookup, tweet_id_lookup} = lookups} =
          if last_insert_cycles > @cycles_to_insert do
            ...
            {max_tweet_depth, tweets} = Enum.reduce(tweets, {-1, %{}}, fn ({depth, cur}, {max_depth, acc}) ->
        ...
        case cur do
          %Model.Tweet{} ->
            ...
            {{cpt_timer, reset_timer, insert_timer}, inserts, lookups}
          %Model.DeletedTweet{} ->
            ...
            {{cpt_timer, reset_timer, insert_timer}, {tweets, tweet_deletes, users, places}, lookups}
          %Model.Limit{} ->
